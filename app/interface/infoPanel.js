...
function updateShips(ships) {
	interfaceCtx.clearRect(
		shipListPositionX,
		textPositionY - shipCfg.heightPx,
		playerCfg.maxLives * (shipCfg.width + 1) * coreCfg.pixelSize,
		shipCfg.heightPx
	);
	...
	for (let i = 0; i < ships.length; i += 1) {
		ships[i].show(
			interfaceCtx,
			shipListPositionX + i * (shipCfg.width + 1) * coreCfg.pixelSize,
			textPositionY - shipCfg.heightPx
		);
	}
}
...
export function init(data) {
	...
	font.write(interfaceCtx, [shipListPositionX, textPositionY], str.ships, {
		size: cfg.fontSize,
		alignment: alignmentConst.right
	});
